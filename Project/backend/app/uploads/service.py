...
async def upload_file(file: UploadFile, folder: str, public: bool = True) -> str:
    ...
    if not settings.upload_provider:
        ...
        return f"data:{file.content_type or 'application/octet-stream'};base64,{encoded}"
    ...
    if settings.upload_provider.lower() == "cloudinary":
        ...
        result = cloudinary.uploader.upload(
            ...
            folder=folder,
            resource_type="auto",
            public_id=os.path.splitext(file.filename or "upload")[0],
        )
    ...
    if settings.upload_provider.lower() == "s3":
        ...
        key = f"{folder}/{os.path.basename(file.filename or 'upload')}"
        client = boto3.client(
            "s3",
        ...
        client.put_object(Bucket=bucket_name, Key=key, Body=file_bytes, ContentType=file.content_type or "application/octet-stream")
        if endpoint_url:
