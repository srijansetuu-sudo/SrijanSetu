...
import { creatorService, notificationService, paymentService, requirementService } from "@/services/api-services";
...
function ProfileOverview({ user, role, creator, unreadNotifications = 0 }) {
  ...
  const completionItems = role === "CREATOR"
    ? [user?.full_name, user?.avatar_url, creator?.brand_name, creator?.headline, creator?.description]
...
export function CreatorDashboardPage() {
  ...
  const payments = useApiQuery(queryKeys.payments, paymentService.history);
  const notifications = useApiQuery(queryKeys.notifications, notificationService.list, { refetchOnMount: "always" });
  const revenue = asArray(payments.data).reduce((sum, payment) => payment.payment_status === "SUCCESS" ? sum + Number(payment.amount ?? 0) : sum, 0);
  return (
    <ProtectedRoute roles={["CREATOR"]}>
      <DashboardShell role="CREATOR">
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-6">
          ...
          <StatCard icon={MessageSquare} label="Workspaces" value="Open" href="/workspaces" />
          <StatCard icon={IndianRupee} label="Revenue" value={money(revenue)} href="/payments" />
          <StatCard icon={Bell} label="Notifications" value={asArray(notifications.data).filter((item) => !item.is_read).length} href="/notifications" />
        </div>
      </DashboardShell>
    </ProtectedRoute>
