...
import { messageService, orderService, paymentService, reviewService } from "@/services/api-services";
...
export function OrderWorkspacePage() {
  ...
  const messages = useApiQuery(queryKeys.messages(id), () => messageService.byOrder(id), { enabled: Boolean(id) });
  ...
  const pay = useApiMutation(() => paymentService.create({ order_id: id, amount: order.data?.total_amount }), { successMessage: "Payment created" });
  ...
  const handleAttachmentFile = (event) => {
    ...
    reader.onload = () => {
      messageForm.setValue("attachment_url", reader.result, { shouldDirty: true, shouldValidate: true });
              ...
              <CardContent>
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 text-muted-foreground">Started {dateLabel(currentOrder.started_at || currentOrder.created_at)}</p>
                  </div>
                ...
                <div className="mt-6 grid gap-2 sm:grid-cols-3">
                  <RoleGuard roles={["CREATOR"]}>{["ACTIVE", "DELIVERED", "COMPLETED"].map((status) => <Button key={status} size="sm" variant="outline" onClick={() => updateStatus.mutate(status)}>{status}</Button>)}</RoleGuard>
                  <RoleGuard roles={["CUSTOMER"]}><Button className="w-full sm:col-span-3" size="sm" variant="accent" onClick={() => pay.mutate()}>Pay Now</Button></RoleGuard>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <CardContent>
              ...
              <form className="mt-4 grid gap-3" onSubmit={messageForm.handleSubmit(submitMessage, showFormValidationToast)}>
                <div className="flex gap-3">
                  ...
                  <label className="grid h-11 w-11 shrink-0 cursor-pointer place-items-center rounded-lg border border-border bg-white text-primary shadow-sm transition-colors hover:bg-blue-50" title="Attach file">
                    <Paperclip className="h-4 w-4" />
                    <input type="file" className="hidden" onChange={handleAttachmentFile} />
                  </label>
                  <Button disabled={sendMessage.isPending}><Send className="h-4 w-4" />Send</Button>
                </div>
                {attachmentUrl ? <p className="flex items-center gap-2 text-xs font-semibold text-muted-foreground"><Paperclip className="h-3.5 w-3.5" />{attachmentName || "Attachment selected"}</p> : null}
              </form>
            </CardContent>
          </Card>
