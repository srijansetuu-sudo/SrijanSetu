...
from app.notifications.schemas import NotificationBulkUpdate, NotificationRead
...
@router.patch("/bulk", response_model=APIResponse)
async def bulk_update_notifications(payload: NotificationBulkUpdate, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)):
    notifications = await service.update_notifications_read_state(db, user, payload.ids, payload.is_read)
    items = [NotificationRead.model_validate(notification).model_dump(mode="json") for notification in notifications]
    return APIResponse(message="Notifications updated", data={"items": items})


@router.patch("/read-all", response_model=APIResponse)
async def mark_all_read(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_active_user)):
    notifications = await service.mark_all_read(db, user)
    items = [NotificationRead.model_validate(notification).model_dump(mode="json") for notification in notifications]
    return APIResponse(message="All notifications marked read", data={"items": items})
